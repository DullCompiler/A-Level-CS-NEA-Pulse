# Pulse Beta 7 | Made by Alexander Richard Dennant | NEA Project for Computer Science
...
class DownloadWindow(): # MainWindow class defines the main window of the application
    ...
    def create_window(self): # create_window method sets up the window and UI components
       ...
       # Gathering Song Data from Database —————————————————————————————————————————————
        ...
        def download_audio(yturl, filepath):
            ...
            if filepath:
                ...
                out_file = video.download(filepath) # type: ignore
                base, ext = os.path.splitext(out_file)
                new_file = base + '.mp3'
                os.rename(out_file, new_file)
                if verbose:
                    print(yt.title + " has been successfully downloaded.")
                cursor.execute("UPDATE Songs SET Song_File_Location = ?, Downloaded = 1 WHERE Youtube_URL = ? AND ID IN (SELECT ID FROM Songs ORDER BY ID DESC LIMIT 5)", (filepath, yturl))
                connection.commit()
                connection.close()
                self.transition_to_mixing_window()
        ...
        if album_art_image: # Load album art for the related song
            ...
            download_audio_button.pack(side="right", padx=(10, 10), pady=(10, 10))
            def mark_rec1_downloaded(recsong1): # mark_rec1_downloaded method marks the first related song as downloaded
                filepath = filedialog.asksaveasfilename()  # Open a file dialog and get the file path
                download_audio(recsong1[6], filepath)  # Pass the filepath as an argument to download_audio() method
        ...
        if album_art_image: # Load album art for the related song
            ...
            def mark_rec2_downloaded(recsong2): # mark_rec2_downloaded method marks the second related song as downloaded
                filepath = filedialog.asksaveasfilename()  # Open a file dialog and get the file path
                download_audio(recsong2[6], filepath)  # Pass the filepath as an argument to download_audio() method
        ...
        if album_art_image: # Load album art for the related song
            ...
            def mark_rec3_downloaded(recsong3): # mark_rec3_downloaded method marks the third related song as downloaded
                filepath = filedialog.asksaveasfilename()  # Open a file dialog and get the file path
                download_audio(recsong3[6], filepath)  # Pass the filepath as an argument to download_audio() method
        ...
        if album_art_image: # Load album art for the related song
            ...
            def mark_rec4_downloaded(recsong4): # mark_rec4_downloaded method marks the fourth related song as downloaded
                filepath = filedialog.asksaveasfilename()  # Open a file dialog and get the file path
                download_audio(recsong4[6], filepath)  # Pass the filepath as an argument to download_audio() method
        ...
        if album_art_image: # Load album art for the related song
            ...
            def mark_rec5_downloaded(recsong5): # mark_rec5_downloaded method marks the fifth related song as downloaded
                filepath = filedialog.asksaveasfilename()  # Open a file dialog and get the file path
                download_audio(recsong5[6], filepath)  # Pass the filepath as an argument to download_audio() method
...
class MixingWindow(): # MainWindow class defines the main window of the application
    ...
    def create_window(self): # create_window method sets up the window and UI components
        ...
        # insert librosa key and bpm collection 
        ...
        album_art_image: CTkImage = load_image_from_url(str(input_song[5])) # type: ignore
        if album_art_image:
            album_art_label = CTkLabel(input_frame, image=album_art_image, text="")
            album_art_label.pack(side="left", padx=(10, 10), pady=(10, 10))

            title_label2 = CTkLabel(input_frame, text=(new_song[1]), font=("Helvetica", 22, "bold"))
            title_label2.pack(anchor="nw", padx=10, pady=10)
            details_label = CTkLabel(input_frame, text=f"Album | {input_song[2]}\nArtist | {input_song[3]}\nGenre | {input_song[4]}\nKey | {self.key_input}\nBPM | {self.bpm_input}", font=("Helvetica", 20), justify="left")
            details_label.pack(anchor="nw", padx=(10, 10))  
        ...
        if album_art_image:
            ...
            title_label2 = CTkLabel(new_song_frame, text=(new_song[1]), font=("Helvetica", 22, "bold"))
            title_label2.pack(anchor="nw", padx=10, pady=10)
            details_label = CTkLabel(new_song_frame, text=f"Album | {new_song[2]}\nArtist | {new_song[3]}\nGenre | {new_song[4]}\nKey | {self.key_new}\nBPM | {self.bpm_new}", font=("Helvetica", 20), justify="left")
            details_label.pack(anchor="nw", padx=(10, 10))        
        ...
        title_label3 = CTkLabel(mixing_frame, text="Mixing Music", font=("Helvetica", 22, "bold"))
        title_label3.pack(anchor="nw", padx=10, pady=10)

        # Insert Key and BPM adjustment, Crossfade Slider, reverb button and mix button
