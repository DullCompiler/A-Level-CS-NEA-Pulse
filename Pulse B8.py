# Pulse Beta 8 | Made by Alexander Richard Dennant | NEA Project for Computer Science
...
class MixingWindow(): # MainWindow class defines the main window of the application
    ...
    def create_window(self): # create_window method sets up the window and UI components
        ...
        # Input Song
        audioread_inp = librosa.core.load(input_song[8]) # Load the input song

        y, sr = audioread_inp # Unpack the audioread tuple into y and sr
        tempo, beat_frames = librosa.beat.beat_track(y=y, sr=sr) # Get the tempo and beat frames of the input song
        self.bpm_input = format(int(round(tempo))) # Set the input song BPM to the tempo
        if verbose: # If verbose mode is enabled, print the input song BPM to the console
            print("Input Song BPM:", self.bpm_input) # Print the input song BPM to the console

        chromagram = librosa.feature.chroma_stft(y=y, sr=sr) # Get the chroma features of the input song
        mean_chroma = np.mean(chromagram, axis=1) # Get the mean chroma of the input song
        ...
        self.key_input = estimated_key
        if verbose: # If verbose mode is enabled, print the estimated key to the console
            print("Detected Key (Input Song):", estimated_key) # Print the estimated key to the console
        
        # New Song
        audioread_new = librosa.core.load(new_song[8]) # Load the new song
        
        y, sr = audioread_new # Unpack the audioread tuple into y and sr
        tempo, beat_frames = librosa.beat.beat_track(y=y, sr=sr) # Get the tempo and beat frames of the new song
        self.bpm_new = format(int(round(tempo))) # Set the new song BPM to the tempo
        if verbose: # If verbose mode is enabled, print the new song BPM to the console
            print("New Song BPM:", self.bpm_new) # Print the new song BPM to the console
            
        chromagram = librosa.feature.chroma_stft(y=y, sr=sr) # Get the chroma features of the new song
        mean_chroma = np.mean(chromagram, axis=1) # Get the mean chroma of the new song
        estimated_key_index = np.argmax(mean_chroma) # Get the index of the estimated key
        estimated_key = chroma_to_key[estimated_key_index] # Get the estimated key
        self.key_new = estimated_key
        if verbose: # If verbose mode is enabled, print the estimated key to the console
            print("Detected Key (New Song):", estimated_key) # Print the estimated key to the console
        ...
        # Export Mix Button
        export_mix_button = CTkButton(mixing_frame, text="Export Mix", font=("Helvetica", 16)) # Create the export mix button
